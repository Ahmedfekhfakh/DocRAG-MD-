...
Judge LLM: Gemini 2.0 Flash (follows structured JSON schemas reliably).
RAG model: controlled by the `model` param in run_ragas_eval (biomistral or gemini).
"""
import asyncio
...
def _get_ragas_llm():
    # Gemini 2.0 Flash as RAGAS judge — reliably follows structured JSON schemas
...
def run_ragas_eval(questions: list[str], model_name: str = "gemini") -> dict:
    ...
    result = evaluate(dataset, metrics=[faithfulness, answer_relevancy], raise_exceptions=False, run_config=RunConfig(max_workers=1, timeout=120))
