...
Boucle Self-RAG : max 2 retries si réponse non fidèle ou incomplète.
"""
...
async def graph_search_node(state: GeneralState, config: RunnableConfig) -> dict:
    """Enrichit avec PrimeKG — relations générales."""
    mode = state.get("mode", "rag")
    ...
    lines = ["[Knowledge Graph — General Relations]"]
    for r in results[:15]:
...
def assemble_node(state: GeneralState) -> dict:
...
async def generate_node(state: GeneralState, config: RunnableConfig) -> dict:
...
async def self_reflect_node(state: GeneralState, config: RunnableConfig) -> dict:
    """Self-RAG : vérifie fidélité/complétude, décide retry ou output."""
...
def should_retry(state: GeneralState) -> str:
...
async def run_general_pipeline(question: str, model_name: str = "gemini", mode: str = "rag", config=None) -> dict:
