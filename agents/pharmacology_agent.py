...
async def query_transform_node(state: PharmacologyState, config: RunnableConfig) -> dict:
...
async def search_node(state: PharmacologyState) -> dict:
...
def rerank_node(state: PharmacologyState) -> dict:
    mode = state.get("mode", "rag")
    if mode == "graph":
